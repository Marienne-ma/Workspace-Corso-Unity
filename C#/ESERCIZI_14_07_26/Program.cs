...
public class Program
{
    static void Main(string[] args)
    {
        try
        {
            ...
            if (!iscritto)
                throw new Exception("Devi essere iscritto alla biblioteca per usufruire dei servizi");

            Console.WriteLine("Quanti libri vuoi prendere? ");
            int libri = Convert.ToInt32(Console.ReadLine());

            if (libri > 3)
                throw new Exception("Puoi prendere massimo 3 libri");
            ...
            if (natale || ferragosto)
                throw new Exception("La biblioteca è chiusa in questo periodo");

            Console.WriteLine("Età: ");
            int eta = Convert.ToInt32(Console.ReadLine());

            if (eta < 18 && iscritto)
                throw new Exception("I minorenni non possono iscriversi");

            Console.WriteLine("Genere del libro: ");
            string genere = Console.ReadLine();

            if ((genere == "Horror" || genere == "Giallo") && eta < 14)
                throw new Exception("Non puoi leggere questo genere");

            Console.WriteLine("La copia è disponibile? (s/n): ");
            bool disponibile = Console.ReadLine().ToLower() == "s";

            if (!disponibile)
                throw new LibroNonDisponibileException("La copia del libro non è disponibile");

            Console.WriteLine("Donazione (5, 10 o 15): ");
            int donazione = Convert.ToInt32(Console.ReadLine());

            if (donazione != 5 && donazione != 10 && donazione != 15)
                throw new DonazioneNonValidaException("La donazione deve essere di 5, 10 o 15 euro");
